...
CREATE TABLE evaluation.runner_submissions (
                                               id              UUID PRIMARY KEY,
...
-- Индексы, которые точно пригодятся для выборок
CREATE INDEX runner_submissions_user_id_idx ON evaluation.runner_submissions (user_id);
CREATE INDEX runner_submissions_question_id_idx ON evaluation.runner_submissions (question_id);
CREATE INDEX runner_submissions_page_id_idx ON evaluation.runner_submissions (page_id);
CREATE INDEX runner_submissions_verdict_idx ON evaluation.runner_submissions (verdict);
CREATE INDEX runner_submissions_submitted_at_idx ON evaluation.runner_submissions (submitted_at);

-- Права сервисному пользователю
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE evaluation.runner_submissions TO evaluation_user;


-- 2. Таблица с AI-review результатов
CREATE TABLE evaluation.runner_ai_reviews (
                                              id           UUID PRIMARY KEY,
...
CREATE INDEX runner_ai_reviews_submission_id_idx ON evaluation.runner_ai_reviews (submission_id);
...
CREATE TABLE evaluation.runner_test_case_results (
                                                     id            UUID PRIMARY KEY,
                                                     submission_id UUID NOT NULL REFERENCES evaluation.runner_submissions (id) ON DELETE CASCADE,
    -- внешний кейс из другой схемы, как в описании
...
CREATE INDEX runner_test_case_results_submission_id_idx ON evaluation.runner_test_case_results (submission_id);
CREATE INDEX runner_test_case_results_test_case_id_idx ON evaluation.runner_test_case_results (test_case_id);
